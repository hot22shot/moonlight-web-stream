...
#[get("/config.js")]
async fn config_js(app: Data<App>, config: Data<Config>) -> HttpResponse {
    let config_json = match serde_json::to_string(&ConfigJs {
        enable_credential_authentication: todo!(),
        path_prefix: config.web_server.url_path_prefix.clone(),
    }) {
        Ok(value) => value,
        Err(err) => {
            warn!(
